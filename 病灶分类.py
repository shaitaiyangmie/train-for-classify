...
#     A.ShiftScaleRotate(),
    ...
    #     A.RandomGamma(),
    #     A.RandomBrightness(),
    #     A.ColorJitter(brightness=0.07, contrast=0.07,saturation=0.1, hue=0.1, always_apply=False, p=0.3),], p=0.3),
    #
    # A.OneOf([
...
#         self.classifier = nn.Linear(512, len(list_train_cate))
#
#     def forward(self, x):
...
for epoch in range(5):
    ...
    if np.mean(list_loss) < loss_mean:
        ...
        torch.save(model.state_dict(), '/Users/shaotianyu01/Desktop/school/11.4/11.11.py_best_loss.pth')
    ...
    with torch.no_grad():
        ...
        if correct / total > acc_epoch:
            ...
            torch.save(model.state_dict(), '/Users/shaotianyu01/Desktop/school/11.4/11.11.py_best_acc.pth')
